...
4. **Run a Claude-powered cover-letter agent** that emits a structured `TailoredCoverLetter` pydantic model. The agent's hard rules forbid fabricating any employer / metric / project, **inventing or recalling any number** (test count, coverage %, LOC, etc.) not stated verbatim in the resume or context, contradicting any resume bullet, and using stale openings ("I am writing to apply", "I am passionate about", "I would welcome the opportunity to"). Body paragraphs (1-4) must each ground in a specific resume bullet. When the caller supplies a **verified-facts** block it is injected ahead of the JD on the first pass, and a deterministic numeric-claim check re-prompts the agent once (no LLM, no loop) if it still contradicts a verified figure.
